...
// 定义一个用于验证和解析 Loon 传入参数的函数
function validateAndParseArguments(args) {
  if (typeof args !== 'object' || args === null) {
    throw new Error('传入的参数必须为对象格式');
  }
  ...
  // 检查必要参数
  for (let key of ['EricPassword', 'scriptvip']) {
    if (!(key in args)) {
...
// 定义一个异步自执行函数
(async () => { 
  // 假设 Loon 的 argument 参数是通过 $argument 传递的
  const rawArguments = $argument; // 直接获取 Loon 传递的参数
  let arguments;

  // 尝试验证和解析参数
  try {
    arguments = validateAndParseArguments(rawArguments); // 验证并转为 JSON 格式
  } catch (error) {
    console.error("参数处理出错:", error.message); // 日志记录
    environmentInstance.msg("参数处理出错", error.message, ""); // 提示信息
    return; // 终止后续逻辑
  }
  ...
  try {
    ...
    const isScriptEnabled = arguments.scriptvip; // 从参数中获取脚本状态
    ...
    function validatePassword(inputPassword, expectedPassword) {
      const encodedPassword = encodeToBase64(inputPassword); // 将传入的密码进行 Base64 编码
      return encodedPassword === expectedPassword; // 验证是否与期望的密码匹配
    }
    ...
    // 这里包含对其他 URL 响应的处理函数…（保持不变）

  } catch (error) {
    console.error("参数处理出错:", error.message); // 处理参数检验时的错误
    environmentInstance.msg("参数处理出错", error.message, ""); // 提示信息
  }
})();
