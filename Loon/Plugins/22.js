...
// 函数：检验并转换 parameters
function validateAndParseArguments(argsStr) {
  let parsedArgs;
  try {
    parsedArgs = JSON.parse(argsStr); // 尝试解析参数为 JSON
  } catch (e) {
    console.error("参数解析失败, 使用默认参数. 错误信息: ", e); // 记录错误信息
    // 提供默认参数
    parsedArgs = { 
      EricPassword: "TG联系咨询",
      scriptvip: "false"
  ...
  return parsedArgs; // 返回解析后的参数
}

// 定义一个异步自执行函数
(async () => { 
  const argsStr = '{"EricPassword": "your_actual_password_here", "scriptvip": "true"}'; // 传入的参数 JSON 字符串
  const arguments = validateAndParseArguments(argsStr); // 验证并转换参数

  try {
    // 解构提取出需要的参数
    const { EricPassword, scriptvip } = arguments; // 从参数中获取密码和脚本状态
    ...
    // 验证密码
    if (!validatePassword(EricPassword, fetchedPassword)) {
